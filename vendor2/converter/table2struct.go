...
type T2tConfig struct {
	...
	ColumnComment    bool // 是否把字段备注设置为注释
}
...
func (t *Table2Struct) Run() error {
	...
	for tableRealName, item := range tableColumns {
		...
		for _, v := range item {
			...
			if t.config.ColumnComment && v.ColumnComment != "" {
...
func (t *Table2Struct) getColumns(table ...string) (tableColumns map[string][]column, err error) {
	...
	for rows.Next() {
		col := column{}
		err = rows.Scan(&col.ColumnName, &col.DataType, &col.ColumnType, &col.Nullable, &col.TableName, &col.CharMaxLen, &col.ColumnComment)
